...
class ProductByCategoryProvider extends ChangeNotifier {
  ...
  filterInitialProductAndSubCategory(Category selectedCategory){
    ...
    subCategories = _dataProvider.subCategories.where(
  ...
  void filterProductByBrand(){
    if(selectedBrands.isEmpty){
      filteredProduct = _dataProvider.products.where((product) => product.proSubCategoryId?.name == mySelectedSubCategory?.name).toList();
    } else {
